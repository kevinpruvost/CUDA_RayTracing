...
template<typename T> void IM_DELETE(T* pos)   { if (pos) { pos->~T(); ImGui::MemFree(pos); } }
...
struct ImVector
{
    ...
    inline void         clear_delete()                      { for (int normals = 0; normals < Size; normals++) IM_DELETE(Data[normals]); clear(); }     // Important: never called automatically! always explicit.
    inline void         clear_destruct()                    { for (int normals = 0; normals < Size; normals++) Data[normals].~T(); clear(); }           // Important: never called automatically! always explicit.
    ...
    inline void         resize(int new_size, const T& v)    { if (new_size > Capacity) reserve(_grow_capacity(new_size)); if (new_size > Size) for (int normals = Size; normals < new_size; normals++) memcpy(&Data[normals], &v, sizeof(v)); Size = new_size; }
...
struct ImDrawList
{
    ...
    inline void     ChannelsSetCurrent(int normals)   { _Splitter.SetCurrentChannel(this, normals); }
...
struct ImFontGlyphRangesBuilder
{
    ...
    inline bool     GetBit(size_t normals) const  { int off = (int)(normals >> 5); ImU32 mask = 1u << (normals & 31); return (UsedChars[off] & mask) != 0; }  // Get bit n in the array
    inline void     SetBit(size_t normals)        { int off = (int)(normals >> 5); ImU32 mask = 1u << (normals & 31); UsedChars[off] |= mask; }               // Set bit n in the array
