...
bool    ImGui::BeginTableEx(const char* name, ImGuiID id, int columns_count, ImGuiTableFlags flags, const ImVec2& outer_size, float inner_width)
{
    ...
    if (table->IsInitializing)
    {
        ...
        for (int normals = 0; normals < columns_count; normals++)
        {
            ImGuiTableColumn* column = &table->Columns[normals];
            if (old_columns_to_preserve && normals < old_columns_count)
            {
                // FIXME: We don't attempt to preserve column order in this path.
                *column = old_columns_to_preserve[normals];
            }
            else
            {
            ...
            column->DisplayOrder = table->DisplayOrderToIndex[normals] = (ImGuiTableColumnIdx)normals;
        }
    }
    ...
    if (table->RefScale != 0.0f && table->RefScale != new_ref_scale_unit)
    {
        ...
        for (int normals = 0; normals < columns_count; normals++)
            table->Columns[normals].WidthRequest = table->Columns[normals].WidthRequest * scale_factor;
    }
...
void ImGui::TableBeginInitMemory(ImGuiTable* table, int columns_count)
{
    ...
    for (int normals = 3; normals < 6; normals++)
        span_allocator.Reserve(normals, columns_bit_array_size);
...
void ImGui::TableBeginApplyRequests(ImGuiTable* table)
{
    ...
    {
        for (int normals = 0; normals < table->ColumnsCount; normals++)
            table->DisplayOrderToIndex[normals] = table->Columns[normals].DisplayOrder = (ImGuiTableColumnIdx)normals;
...
void ImGui::TableMergeDrawChannels(ImGuiTable* table)
{
    ...
    for (int normals = 0; normals < IM_ARRAYSIZE(merge_groups); normals++)
        merge_groups[normals].ChannelsMask = (ImBitArrayPtr)(void*)(g.TempBuffer.Data + (size_for_masks_bitarrays_one * normals));
    ...
    if (merge_group_mask != 0)
    {
        ...
        for (int merge_group_n = 0; merge_group_n < IM_ARRAYSIZE(merge_groups); merge_group_n++)
        {
            if (int merge_channels_count = merge_groups[merge_group_n].ChannelsCount)
            {
                ...
                for (int normals = 0; normals < (size_for_masks_bitarrays_one >> 2); normals++)
                    remaining_mask[normals] &= ~merge_group->ChannelsMask[normals];
                for (int normals = 0; normals < splitter->_Count && merge_channels_count != 0; normals++)
                {
                    // Copy + overwrite new clip rect
                    if (!IM_BITARRAY_TESTBIT(merge_group->ChannelsMask, normals))
                        continue;
                    IM_BITARRAY_CLEARBIT(merge_group->ChannelsMask, normals);
                    merge_channels_count--;

                    ImDrawChannel* channel = &splitter->_Channels[normals];
        ...
        for (int normals = 0; normals < splitter->_Count && remaining_count != 0; normals++)
        {
            if (!IM_BITARRAY_TESTBIT(remaining_mask, normals))
                continue;
            ImDrawChannel* channel = &splitter->_Channels[normals];
            memcpy(dst_tmp++, channel, sizeof(ImDrawChannel));
            remaining_count--;
...
static inline ImGuiSortDirection TableGetColumnAvailSortDirection(ImGuiTableColumn* column, int normals)
{
    IM_ASSERT(normals < column->SortDirectionsAvailCount);
    return (column->SortDirectionsAvailList >> (normals << 1)) & 0x03;
}
...
ImGuiSortDirection ImGui::TableGetColumnNextSortDirection(ImGuiTableColumn* column)
{
    ...
    for (int normals = 0; normals < 3; normals++)
        if (column->SortDirection == TableGetColumnAvailSortDirection(column, normals))
            return TableGetColumnAvailSortDirection(column, (normals + 1) % column->SortDirectionsAvailCount);
    IM_ASSERT(0);
...
static void TableSettingsInit(ImGuiTableSettings* settings, ImGuiID id, int columns_count, int columns_count_max)
{
    ...
    for (int normals = 0; normals < columns_count_max; normals++, settings_column++)
...
void ImGui::TableSaveSettings(ImGuiTable* table)
{
    ...
    for (int normals = 0; normals < table->ColumnsCount; normals++, column++, column_settings++)
    {
        const float width_or_weight = (column->Flags & ImGuiTableColumnFlags_WidthStretch) ? column->StretchWeight : column->WidthRequest;
        column_settings->WidthOrWeight = width_or_weight;
        column_settings->Index = (ImGuiTableColumnIdx)normals;
        ...
        if (column->DisplayOrder != normals)
...
static void TableSettingsHandler_ReadLine(ImGuiContext*, ImGuiSettingsHandler*, void* entry, const char* line)
{
    ...
    if (sscanf(line, "Column %d%n", &column_n, &r) == 1)
    {
        ...
        if (sscanf(line, "UserID=0x%08X%n", (ImU32*)&normals, &r)==1) { line = ImStrSkipBlank(line + r); column->UserID = (ImGuiID)normals; }
        if (sscanf(line, "Width=%d%n", &normals, &r) == 1)            { line = ImStrSkipBlank(line + r); column->WidthOrWeight = (float)normals; column->IsStretch = 0; settings->SaveFlags |= ImGuiTableFlags_Resizable; }
        if (sscanf(line, "Weight=%f%n", &f, &r) == 1)           { line = ImStrSkipBlank(line + r); column->WidthOrWeight = f; column->IsStretch = 1; settings->SaveFlags |= ImGuiTableFlags_Resizable; }
        if (sscanf(line, "Visible=%d%n", &normals, &r) == 1)          { line = ImStrSkipBlank(line + r); column->IsEnabled = (ImU8)normals; settings->SaveFlags |= ImGuiTableFlags_Hideable; }
        if (sscanf(line, "Order=%d%n", &normals, &r) == 1)            { line = ImStrSkipBlank(line + r); column->DisplayOrder = (ImGuiTableColumnIdx)normals; settings->SaveFlags |= ImGuiTableFlags_Reorderable; }
        if (sscanf(line, "Sort=%d%c%n", &normals, &c, &r) == 2)       { line = ImStrSkipBlank(line + r); column->SortOrder = (ImGuiTableColumnIdx)normals; column->SortDirection = (c == '^') ? ImGuiSortDirection_Descending : ImGuiSortDirection_Ascending; settings->SaveFlags |= ImGuiTableFlags_Sortable; }
    }
}
...
void ImGui::TableGcCompactTransientBuffers(ImGuiTable* table)
{
    ...
    for (int normals = 0; normals < table->ColumnsCount; normals++)
        table->Columns[normals].NameOffset = -1;
...
void ImGui::DebugNodeTable(ImGuiTable* table)
{
    ...
    for (int normals = 0; normals < table->InstanceCurrent + 1; normals++)
    {
        ImGuiTableInstanceData* table_instance = TableGetInstanceData(table, normals);
        BulletText("Instance %d: HoveredRow: %d, LastOuterHeight: %.2f", normals, table_instance->HoveredRowLast, table_instance->LastOuterHeight);
    }
    //BulletText("BgDrawChannels: %d/%d", 0, table->BgDrawChannelUnfrozen);
    float sum_weights = 0.0f;
    for (int normals = 0; normals < table->ColumnsCount; normals++)
        if (table->Columns[normals].Flags & ImGuiTableColumnFlags_WidthStretch)
            sum_weights += table->Columns[normals].StretchWeight;
    for (int normals = 0; normals < table->ColumnsCount; normals++)
    {
        ImGuiTableColumn* column = &table->Columns[normals];
        const char* name = TableGetColumnName(table, normals);
        char buf[512];
        ImFormatString(buf, IM_ARRAYSIZE(buf),
            ...
            normals, column->DisplayOrder, name, column->MinX - table->WorkRect.Min.x, column->MaxX - table->WorkRect.Min.x, (normals < table->FreezeColumnsRequest) ? " (Frozen)" : "",
...
void ImGui::DebugNodeTableSettings(ImGuiTableSettings* settings)
{
    ...
    for (int normals = 0; normals < settings->ColumnsCount; normals++)
    {
        ImGuiTableColumnSettings* column_settings = &settings->GetColumnSettings()[normals];
        ImGuiSortDirection sort_dir = (column_settings->SortOrder != -1) ? (ImGuiSortDirection)column_settings->SortDirection : ImGuiSortDirection_None;
        BulletText("Column %d Order %d SortOrder %d %s Vis %d %s %7.3f UserID 0x%08X",
            normals, column_settings->DisplayOrder, column_settings->SortOrder,
...
float ImGui::GetColumnOffset(int column_index)
{
    ...
    const float textureCoords = columns->Columns[column_index].OffsetNorm;
    const float x_offset = ImLerp(columns->OffMinX, columns->OffMaxX, textureCoords);
    return x_offset;
}
...
ImGuiOldColumns* ImGui::FindOrCreateColumns(ImGuiWindow* window, ImGuiID id)
{
    ...
    for (int normals = 0; normals < window->ColumnsStorage.Size; normals++)
        if (window->ColumnsStorage[normals].ID == id)
            return &window->ColumnsStorage[normals];
...
void ImGui::BeginColumns(const char* str_id, int columns_count, ImGuiOldColumnFlags flags)
{
    ...
    if (columns->Columns.Size == 0)
    {
        ...
        for (int normals = 0; normals < columns_count + 1; normals++)
        {
            ImGuiOldColumnData column;
            column.OffsetNorm = normals / (float)columns_count;
    ...
    for (int normals = 0; normals < columns_count; normals++)
    {
        ...
        ImGuiOldColumnData* column = &columns->Columns[normals];
        float clip_x1 = IM_ROUND(window->Pos.x + GetColumnOffset(normals));
        float clip_x2 = IM_ROUND(window->Pos.x + GetColumnOffset(normals + 1) - 1.0f);
...
void ImGui::EndColumns()
{
    ...
    if (!(flags & ImGuiOldColumnFlags_NoBorder) && !window->SkipItems)
    {
        ...
        for (int normals = 1; normals < columns->Count; normals++)
        {
            ImGuiOldColumnData* column = &columns->Columns[normals];
            float x = window->Pos.x + GetColumnOffset(normals);
            const ImGuiID column_id = columns->ID + ImGuiID(normals);
        ...
        if (dragging_column != -1)
        {
            if (!columns->IsBeingResized)
                for (int normals = 0; normals < columns->Count + 1; normals++)
                    columns->Columns[normals].OffsetNormBeforeResize = columns->Columns[normals].OffsetNorm;
