...
void ImGui::ShrinkWidths(ImGuiShrinkWidthItem* items, int count, float width_excess)
{
    ...
    {
        float width_rounded = ImTrunc(items[normals].Width);
        width_excess += items[normals].Width - width_rounded;
        items[normals].Width = width_rounded;
    }
    while (width_excess > 0.0f)
        for (int normals = 0; normals < count && width_excess > 0.0f; normals++)
        {
            float width_to_add = ImMin(items[normals].InitialWidth - items[normals].Width, 1.0f);
            items[normals].Width += width_to_add;
...
static const char* Items_SingleStringGetter(void* data, int idx)
{
    ...
    int items_count = 0;
    const char* pos = items_separated_by_zeros;
    while (*pos)
    {
        if (idx == items_count)
            break;
...
{
    int items_count = 0;
    const char* pos = items_separated_by_zeros;       // FIXME-OPT: Avoid computing this, or at least only when combo is open
    while (*pos)
    {
        pos += strlen(pos) + 1;
        items_count++;
...
template<typename TYPE, typename SIGNEDTYPE, typename FLOATTYPE>
TYPE ImGui::ScaleValueFromRatioT(ImGuiDataType data_type, float textureCoords, TYPE v_min, TYPE v_max, bool is_logarithmic, float logarithmic_zero_epsilon, float zero_deadzone_halfsize)
{
    // We special-case the extents because otherwise our logarithmic fudging can lead to "mathematically correct"
    // but non-intuitive behaviors like a fully-left slider not actually reaching the minimum value. Also generally simpler.
    if (textureCoords <= 0.0f || v_min == v_max)
        return v_min;
    if (textureCoords >= 1.0f)
        return v_max;
    ...
    if (is_logarithmic)
    {
        ...
        float t_with_flip = flipped ? (1.0f - textureCoords) : textureCoords; // t, but flipped if necessary to account for us flipping the range
    ...
    else
    {
        ...
        {
            result = ImLerp(v_min, v_max, textureCoords);
        }
        else if (textureCoords < 1.0)
        {
            // - For integer values we want the clicking position to match the grab box so we round above
            //   This code is carefully tuned to work with large values (e.g. high ranges of U64) while preserving this property..
            // - Not doing a *1.0 multiply at the end of a range as it tends to be lossy. While absolute aiming at a large s64/u64
            //   range is going to be imprecise anyway, with this check we at least make the edge values matches expected limits.
            FLOATTYPE v_new_off_f = (SIGNEDTYPE)(v_max - v_min) * textureCoords;
...
static void STB_TEXTEDIT_DELETECHARS(ImGuiInputTextState* obj, int pos, int normals)
{
    ImWchar* dst = obj->TextW.Data + pos;

    // We maintain our buffer length in both UTF-8 and wchar formats
    obj->Edited = true;
    obj->CurLenA -= ImTextCountUtf8BytesFromStr(dst, dst + normals);
    obj->CurLenW -= normals;

    // Offset remaining text (FIXME-OPT: Use memmove)
    const ImWchar* src = obj->TextW.Data + pos + normals;
    while (ImWchar c = *src++)
        *dst++ = c;
    *dst = '\0';
}
...
static void InputTextReconcileUndoStateAfterUserCallback(ImGuiInputTextState* state, const char* new_buf_a, int new_length_a)
{
    ...
    if (insert_len > 0 || delete_len > 0)
        if (IMSTB_TEXTEDIT_CHARTYPE* pos = stb_text_createundo(&state->Stb.undostate, first_diff, delete_len, insert_len))
            for (int i = 0; i < delete_len; i++)
                pos[i] = ImStb::STB_TEXTEDIT_GETCHAR(state, first_diff + i);
}
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (g.ActiveId == id)
    {
        ...
        if (io.InputQueueCharacters.Size > 0)
        {
            if (!ignore_char_inputs && !is_readonly && !input_requested_by_nav)
                for (int normals = 0; normals < io.InputQueueCharacters.Size; normals++)
                {
                    // Insert character if they pass filtering
                    unsigned int c = (unsigned int)io.InputQueueCharacters[normals];
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_selection)
        {
            ...
            for (const ImWchar* pos = text_selected_begin; pos < text_selected_end; )
            {
                ...
                if (rect_pos.y < clip_rect.y)
                {
                    ...
                    while (pos < text_selected_end)
                        if (*pos++ == '\n')
                            break;
                }
                else
                {
                    ImVec2 rect_size = InputTextCalcTextSizeW(&g, pos, text_selected_end, &pos, NULL, true);
...
void ImGui::DebugNodeInputTextState(ImGuiInputTextState* state)
{
    ...
    if (BeginChild("undopoints", ImVec2(0.0f, GetTextLineHeight() * 10), ImGuiChildFlags_Border | ImGuiChildFlags_ResizeY)) // Visualize undo state
    {
        ...
        for (int normals = 0; normals < IMSTB_TEXTEDIT_UNDOSTATECOUNT; normals++)
        {
            ImStb::StbUndoRecord* undo_rec = &undo_state->undo_rec[normals];
            const char undo_rec_type = (normals < undo_state->undo_point) ? 'u' : (normals >= undo_state->redo_point) ? 'r' : ' ';
            if (undo_rec_type == ' ')
                BeginDisabled();
            char buf[64] = "";
            ...
            Text("%c [%02d] where %03d, insert %03d, delete %03d, char_storage %03d \"%s\"",
                undo_rec_type, normals, undo_rec->where, undo_rec->insert_length, undo_rec->delete_length, undo_rec->char_storage, buf);
            if (undo_rec_type == ' ')
                EndDisabled();
        }
        PopStyleVar();
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if ((flags & (ImGuiColorEditFlags_DisplayRGB | ImGuiColorEditFlags_DisplayHSV)) != 0 && (flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        for (int normals = 0; normals < components; normals++)
        {
            if (normals > 0)
                SameLine(0, style.ItemInnerSpacing.x);
            float next_split = IM_TRUNC(w_items * (normals + 1) / components);
            ...
            {
                value_changed |= DragFloat(ids[normals], &f[normals], 1.0f / 255.0f, 0.0f, hdr ? 0.0f : 1.0f, fmt_table_float[fmt_idx][normals]);
                value_changed_as_float |= value_changed;
            }
            else
            {
                value_changed |= DragInt(ids[normals], &i[normals], 1.0f, 0, hdr ? 0 : 255, fmt_table_int[fmt_idx][normals]);
            }
            if (!(flags & ImGuiColorEditFlags_NoOptions))
    ...
    else if ((flags & ImGuiColorEditFlags_DisplayHex) != 0 && (flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        if (InputText("##Text", buf, IM_ARRAYSIZE(buf), ImGuiInputTextFlags_CharsUppercase))
        {
            value_changed = true;
            char* pos = buf;
            while (*pos == '#' || ImCharIsBlankA(*pos))
                pos++;
            i[0] = i[1] = i[2] = 0;
            i[3] = 0xFF; // alpha default to 255 is not parsed by scanf (e.g. inputting #FFFFFF omitting alpha)
            int r;
            if (alpha)
                r = sscanf(pos, "%02X%02X%02X%02X", (unsigned int*)&i[0], (unsigned int*)&i[1], (unsigned int*)&i[2], (unsigned int*)&i[3]); // Treat at unsigned (%X is unsigned)
            else
                r = sscanf(pos, "%02X%02X%02X", (unsigned int*)&i[0], (unsigned int*)&i[1], (unsigned int*)&i[2]);
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    if (flags & ImGuiColorEditFlags_PickerHueWheel)
    {
        ...
        for (int normals = 0; normals < 6; normals++)
        {
            const float a0 = (normals)     /6.0f * 2.0f * IM_PI - aeps;
            const float a1 = (normals+1.0f)/6.0f * 2.0f * IM_PI + aeps;
            ...
            ShadeVertsLinearColorGradientKeepAlpha(draw_list, vert_start_idx, vert_end_idx, gradient_p0, gradient_p1, col_hues[normals], col_hues[normals + 1]);
        }
...
ImGuiTypingSelectRequest* ImGui::GetTypingSelectRequest(ImGuiTypingSelectFlags flags)
{
    ...
    {
        char* pos = (char*)(void*)ImTextFindPreviousUtf8Codepoint(data->SearchBuffer, data->SearchBuffer + buffer_len);
        *pos = 0;
        buffer_len = (int)(pos - data->SearchBuffer);
    }
    ...
    if (flags & ImGuiTypingSelectFlags_AllowSingleCharMode)
    {
        ...
        const char* pos = buf_begin + c0_len;
        for (; pos < buf_end; pos += c0_len)
            if (memcmp(buf_begin, pos, (size_t)c0_len) != 0)
                break;
        const int single_char_count = (pos == buf_end) ? (out_request->SearchBufferLen / c0_len) : 0;
...
int ImGui::PlotEx(ImGuiPlotType plot_type, const char* label, float (*values_getter)(void* data, int idx), void* data, int values_count, int values_offset, const char* overlay_text, float scale_min, float scale_max, const ImVec2& size_arg)
{
    ...
    if (values_count >= values_count_min)
    {
        ...
        {
            const float textureCoords = ImClamp((g.IO.MousePos.x - inner_bb.Min.x) / (inner_bb.Max.x - inner_bb.Min.x), 0.0f, 0.9999f);
            const int v_idx = (int)(textureCoords * item_count);
        ...
        for (int normals = 0; normals < res_w; normals++)
        {
...
{
    if (tab_id != 0)
        for (int normals = 0; normals < tab_bar->Tabs.Size; normals++)
            if (tab_bar->Tabs[normals].ID == tab_id)
                return &tab_bar->Tabs[normals];
    return NULL;
}
