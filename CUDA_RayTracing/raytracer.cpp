...
Color Raytracer::CalnReflection(CollidePrimitive collide_primitive , Vector3 ray_V , int dep , int* hash )
{
    ...
    // diffuse reflection (fuzzy reflection)
        ...
        Color ret;
        int num_samples = 16; // Number of samples for diffuse reflection
        double drefl = primitive->GetMaterial()->drefl;
        ...
        for (int i = 0; i < num_samples; ++i) {
            ...
            double theta = acos(1 - dis(gen) * (1 - cos(drefl)));
            ...
            // Create a perturbation vector in local coordinate system
            Vector3 perturbation(x, y, z);

            // Convert perturbation to world coordinate system
            Vector3 perturbed_ray_V = (ray_V + perturbation * drefl).GetUnitVector();

            // Trace the perturbed ray
            ret += RayTracing(collide_primitive.C, perturbed_ray_V, dep + 1, hash);
        }
...
void Raytracer::CreateAll()
{
    ...
    while ( fin >> obj ) {
        ...
        if ( obj == "primitive" ) {
            ...
            if (type == "mesh") new_primitive = new Mesh;
