...
__device__ bool IntersectInfiniteCylinder(const double3* origin, const double3* direction, Cuda_Bezier* bezier, double3* intersectionPoint)
{
    ...
    if (proj1 >= 0.0 && proj1 <= length(O2 - O1))
        *intersectionPoint = P2; // Intersection outside the segment
    else
        *intersectionPoint = P1; // Intersection outside the segment
...
__device__ bool IntersectBezierCurve2D(const double3 * origin, const double3 * direction, const double3 * P, const double3& R1, const double3& R2, double3* R, int degree, double3* intersectionPoint, Cuda_Bezier* bezier)
{
    ...
    double2 direction2D = make_double2(dot(*direction, axisX), dot(*direction, axisY));
...
__device__ void BezierIntersect(Cuda_Primitive* primitive, const double3* origin, const double3* direction, Cuda_Collision* collision)
{
    ...
    double3 intersectionPointCylinder;
    if (IntersectInfiniteCylinder(origin, direction, bezier, &intersectionPointCylinder))
    {
